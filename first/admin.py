...
class ParticipantAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'pc_no', 'username', 'score_display', 'status_display',
        'eligible_display', 'hints_display', 'submitted_display',
    )
    ...
    readonly_fields = (
        ...
        'best_score', 'last_login', 'score_display', 'status_display',
    ...
    fields = (
        'pc_no', 'username', 'registered_at', 'game_start_time', 'completed_at',
        'score_display', 'status_display', 'eligible_display', 'hints_display',
        'submitted_display', 'judging_links', 'is_admin', 'is_active',
    )

    @admin.display(description='Score', ordering='best_score')
    def score_display(self, obj):
        return _score_label(obj.best_score, TOTAL_CHECKS)

    @admin.display(description='Status')
    def status_display(self, obj):
        if not obj.game_start_time:
            return 'Not started'
        if not obj.is_expired:
            return 'Design mode' if obj.design_mode else 'In progress'
        return 'Submitted' if obj.is_eligible else 'Expired'
